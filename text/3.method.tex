...
succeeds. This is deliberately generous, and \S\ref{sec:disagreement} shows that how
many layers go into the union changes the result quite a lot.
...
Neither readout is ground truth for the other, and they are not two ways of
decoding the same thing. Patchscopes works on a single hidden state and asks
what that vector can be made to say; generation runs the whole model on the
matched text and asks what it does. One measures what can be pulled out of a
representation, the other what the model actually does, so we treat them as two
...
What a success shows is that the state carries enough information to bring the
continuation out under that readout. We use \emph{recognition} and
\emph{anticipation} in that practical sense throughout: a phrase is recognized
at distance $i$ when a named readout recovers its remaining $i$ tokens under the
matching rule below.
...
trial) while staying tight enough that a success reflects the phrase rather than a
long ramble that happens to contain it.\footnote{An earlier
