...
without natural context. Context lifts both readouts by $20$--$36$ points; the
two readouts themselves track each other closely once the Patchscopes union is
taken over all $32$ layers.}
\label{fig:context}
\end{figure}
...
$i=5$. This is the expected direction --- context supplies both topical evidence
and the signal that a fixed expression is under way --- but its size matters for
interpreting prior results. A bare phrase is a poor proxy for the condition
...
point, same success criterion --- and they agree on $81.1\%$ of the $24{,}740$
pooled trials.

The residual $18.9\%$ is the informative part, because it is not one-sided.
$1{,}924$ instances are recovered by Patchscopes and not by generation, and
$2{,}751$ by generation and not by Patchscopes. These are different kinds of
error. In the first, the state does contain the phrase --- the patchscope
verbalises it --- but the real context steers the model elsewhere; given
\textit{Why should one person be brought up}, the model writes \textit{a wealthy
family, with every opportunity} rather than completing \textit{in the lap of
luxury}. In the second, the model completes the phrase correctly from its real
prefix while the patchscope, at the same position, produces a related expression
instead: for \textit{a bird in the hand is worth two in the bush} it returns
\textit{worth more than one}. The first is a failure of the continuation to be
the one we asked about; the second is a failure of the instrument.

\paragraph{The size of the gap depends on how many layers are read.}
Restricting the Patchscopes union to the eight layers used for probing drops it
from $79.6\%$ to $73.2\%$ at $i=2$ and from $46.4\%$ to $38.7\%$ at $i=5$
(Table~\ref{tab:main}, last row). Under that restriction, generation appears to
outperform Patchscopes by $7$ to $15$ points and the gap appears to grow with
$i$; under the full $32$-layer sweep the gap is $0.7$ to $7.2$ points and is
essentially absent at $i=3$. The apparent superiority of one readout over the
other is thus largely a function of how thoroughly the other was sampled, which
is a reason to report the layer budget explicitly whenever a Patchscopes rate is
quoted.

\paragraph{Category difficulty is readout-dependent.}
Table~\ref{tab:categories} breaks the pooled rates down by phrase kind. Under
generation, film titles are recovered most often ($84.5\%$), landmarks next
($75.3\%$) and idioms least ($66.4\%$). Under Patchscopes the ordering differs:
film titles again lead ($68.6\%$) but idioms ($65.9\%$) sit above landmarks
($56.8\%$), and the spread between best and worst narrows from $18.1$ to $11.8$
points. Idioms are the category whose relative standing moves most, which is
consistent with the scoring artifact of \S\ref{sec:falsepos}: idioms have
accepted variants, so exact-match scoring penalises them under a readout that
produces fluent continuations more than under one that produces fragments.

The practical consequence is narrow but real. A study that measured only with
Patchscopes would report that this model finds idioms about as easy as
landmarks; one that measured only with generation would report idioms as clearly
the hardest category. Both would be reporting their instrument.
