...
throughout, and both decay steeply. The number of phrases long enough to test
falls with $i$, from $1{,}112$ at $i=2$ to $51$ at $i=8$. \textbf{(b)} The same
...
network, peaks at L25 and falls back at L30
(Figure~\ref{fig:bylayer}, Appendix~\ref{app:details}). One explanation is that
the last layers specialise toward producing the actual next token rather than
maintaining a transferable representation, so a state lifted from L30 transfers
worse than one from L25 --- a hypothesis consistent with our data and with the
late-layer decline \citet{kaplan-etal-2025-tokens} report for words, not
something we establish. Over all $32$ layers the median first successful layer
is L16, and $41.9\%$ of successes first appear at or before L10: phrase
information becomes readable gradually, and for a large minority quite early.
...
same probe on the earliest layers gives a clean gradient: $58.6\%$ at L0 --- the
raw embedding --- rising through $65.1\%$ at L1 and $66.5\%$ at L2 to $69.8\%$
at L5 (Appendix~\ref{app:details}). A base-rate artifact would be flat; this
...
the probe's guess --- true positives average $0.904$ and false negatives
$0.791$, against $0.599$ for false positives and $0.414$ for true negatives
