...
The observed distribution puts mass at both ends --- $7.7\%$ of phrases fail
everywhere and $31.5\%$ succeed everywhere --- where the null puts $0.3\%$ and
$10.1\%$; the null instead piles up in the middle, where the observed
...
One explanation is that the last layers specialise toward producing the actual
next token rather than maintaining a transferable representation, so a state
lifted from L30 transfers worse than one from L25. This is a hypothesis
consistent with our data and with the late-layer decline
\citet{kaplan-etal-2025-tokens} report for words, not something we establish: a
state a patchscope cannot verbalise may still carry the information in a form
...
The two failure directions are qualitatively different, and neither is simply an
instrument defect (Table~\ref{tab:disagree}). Where Patchscopes succeeds and
generation does not, the state does contain the phrase --- the patchscope
verbalises it --- yet the real context steers the model elsewhere: given
...
rather than as a corrected accuracy. Read that way, it says roughly four in five
of these apparent errors reflect the rigidity of exact matching rather than a
model that did not know the phrase --- which makes our reported rates
conservative.
