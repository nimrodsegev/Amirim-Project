...
Table~\ref{tab:falsepos} gives examples: \textit{laughs best} for
\textit{laughs loudest}, \textit{plowshares} for \textit{ploughshares},
\textit{beat around the bush} for \textit{beat about the bush} --- in each case
the more common form of the expression, scored as a failure.
...
points the same way. In $40.0\%$ of them the model was itself confident
($>0.7$), which is the signature of a valid alternate; in only $10.4\%$ was it
unsure ($<0.3$), the signature of real confusion. The remaining $49.6\%$ fall in
between. This uses no judge at all, and agrees with the adjudication that most
of these are not failures of knowledge.
...
generates over the steps needed to complete the phrase, mean confidence is
$0.878$ on successful completions and $0.447$ on failures (medians $0.934$ and
$0.416$) --- a usable signal in its own right. Split by probe outcome at L25
(Figure~\ref{fig:conf}, left), confidence follows the true label rather than the
probe's guess: true positives average $0.904$ and false negatives $0.791$, while
false positives average $0.599$ and true negatives $0.414$. The ordering is by
what the model actually did, not by what the probe predicted. Where the probe is
wrong, the model was not, which suggests the two signals are partly complementary
and that combining them is worth trying.
