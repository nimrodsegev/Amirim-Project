...
If the remainder were already recoverable from the state at \textit{Lord}, most
of that work would be redundant. Whether it is recoverable is the question of
this paper; we do not implement the shortcut.}
...
a binomial expectation of $10.1\%$ (\S\ref{sec:stability}). And the signal is decodable: a small classifier
reading the raw hidden state predicts whether the model will in fact complete
the phrase, at $76.8\%$ balanced accuracy and $87.6\%$ precision
(\S\ref{sec:probe}).
...
(\S\ref{sec:disagreement}). They also reverse the ranking of the phrase types
--- idioms are the easiest under Patchscopes and the hardest under generation
--- and the two probes trained on their respective labels place the ``best''
layer in different parts of the network. Some of the asymmetry is an artifact of
how many layers the Patchscopes union is taken over, and we quantify how much;
the disagreement that remains is not, and survives the widest sweep we can run.
