...
2. **Evidence that recovery varies systematically by phrase rather than by occasion.** Over the 803 phrases with ≥5 *distinct* contexts, per-phrase success at `i=3` is far more bimodal than an independent per-context process at a common rate: 31.5% succeed in ≥90% of their contexts (binomial null 10.1%) and 18.3% in ≤20% (null 5.0%). Note the scope: this shows outcomes cluster by phrase, **not** that phrase identity causes the clustering, since frequency, length, tokenization, category and context similarity are not separated.
3. **Evidence that the readout changes the conclusion.** On the *identical* 24,740 (phrase, context, `i`) instances the two readouts agree on 78.7%, and where they disagree it is directional: generation recovers what patchscopes misses ~3× as often as the reverse, so a patchscopes rate reads as a floor rather than an estimate. Widening the sweep to all 32 layers accounts for much of that gap (agreement 81.1%, ratio ~3:2). What survives is layer localisation: the same probe peaks mid-network on one readout's labels and at L30 on the other's. The category-ranking reversal is a concrete illustration in §5.3, not a headline.
4. **A hidden-state probe that predicts whether the model will complete the phrase**, at 76.8% balanced accuracy and 87.6% precision (L30), and well above chance already at L1. Caveat: ~80% of its apparent false positives are acceptable variants rather than mistakes, but those are the probe's positives at one layer, not a sample of all failures, so this does not correct the headline rates.
...
| Recovery clusters by phrase rather than by occasion | Per-phrase success distribution vs a binomial null | `per_phrase_consistency_i3` | Computed at `i=3` over the 803 phrases with ≥5 **distinct** contexts; raw instance counts must not be used here (44.8% are duplicates) |
